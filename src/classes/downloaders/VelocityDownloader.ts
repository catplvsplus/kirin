...
export class VelocityDownloader {
    public versions: Collection<string, string[]> = new Collection();
    public builds: Collection<string, VelocityDownloader.BuildResponseData[]> = new Collection();
    ...
    public async fetchVersions(): Promise<Collection<string, string[]>> {
        ...
        const response = await fetch('https://fill.papermc.io/v3/projects/velocity');
        if (!response.ok) throw Error('Failed to fetch versions', { cause: response });

        const data: VelocityDownloader.VersionsResponseData = await response.json();
    ...
    public async fetchBuilds(version: string): Promise<VelocityDownloader.BuildResponseData[]> {
        if (this.builds.has(version)) return this.builds.get(version)!;

        const response = await fetch(`https://fill.papermc.io/v3/projects/velocity/versions/${version}/builds`);
        if (!response.ok) throw Error('Failed to fetch builds', { cause: response });

        const data: VelocityDownloader.BuildResponseData[] = await response.json();

        this.builds.set(version, data);
        return data;
    }

    public async fetchLatestBuild(): Promise<VelocityDownloader.BuildResponseData|null> {
...
export namespace VelocityDownloader {
